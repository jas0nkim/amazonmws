...
def main(argv):
    ...
    try:
        ...
        print 'hotfix_variations.py -s <basic|premium>'
        sys.exit(2)
...
def scrape_amazon(premium, task_id, ebay_store_id):
    ...
    asins = EbayItemModelManager.fetch_distinct_asin(ebay_store_id=ebay_store_id)

    # scrape amazon items (variations)
    if len(asins) > 0:
        process = CrawlerProcess(get_project_settings())
        process.crawl('amazon_asin', asins=asins, task_id=task_id, ebay_store_id=ebay_store_id, premium=premium)
        process.start()
    else:
