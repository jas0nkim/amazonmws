...
def __update_order_status_if_exists(ebay_store, since_hours_ago=4):
    ...
    for moded_order in orders:
        try:
            ...
            if _existed_order.order_status == moded_order.OrderStatus and _existed_order.payment_status == moded_order.MonetaryDetails.Payments.Payment.PaymentStatus:
                continue

            ebay_order = EbayOrderModelManager.update(order=_existed_order, 
                order_status=moded_order.OrderStatus,
                payment_status=moded_order.MonetaryDetails.Payments.Payment.PaymentStatus)
