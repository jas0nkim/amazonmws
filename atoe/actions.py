...
class EbayOauthAction(object):
    ...
    def update_user_access(self, refresh_token):
        ...
        try:
            ...
            logger.exception("failed to update user access - {}".format(str(e)))
            print("failed to update user access - {}".format(str(e)))
        except:
            pass
        return ret
...
class EbayInventoryLocationAction(BaseEbayInventoryAction):
    ...
    def create_inventory_location(self, merchant_location_key, https_payload):
        ret = False
        try:
            if not merchant_location_key:
                raise Exception('No merchant location key passed')
