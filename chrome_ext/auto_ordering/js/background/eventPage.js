...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (isAutomationJTab(tab)) { // automationj tab
        if (changeInfo.status == "complete") {
            if (tab.url.match(/^http:\/\/172\.104\.12\.103:8092\/orders\/all\//)) {
            ...
            } else if (tab.url.match(/^http:\/\/172\.104\.12\.103:8092\/orders\/unsourced\//)) {
                chrome.tabs.executeScript(tabId, { file: 'js/contentscripts/automationj/orders.js' },
                    function() {
                        chrome.tabs.sendMessage(
                            tab.id,
